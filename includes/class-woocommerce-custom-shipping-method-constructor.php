			...
			*/
			public function calculate_shipping( $package = array() ) {
				...
				if(isset($response['rows'][0]) && $response['rows'][0]['elements'][0]['status'] !== 'NOT_FOUND' && $response['rows'][0]['elements'][0]['status'] !== 'ZERO_RESULTS') {
					...
					$new_extraprice = 0;
					if($setDistance <= 80 and $price < 200) {
						$percentage = 20;
						$price = $price;						
						$new_extraprice = ($percentage / 100) * $price;						
							} 
					else if ($setDistance > 80 and $setDistance <= 200 and $price < 200) 
						{
						$percentage = 25;
						$price = $price;						
						$new_extraprice = ($percentage / 100) * $price;	
					 ...
					 if($setDistance <= 80 and $price >= 200 and $price < 500) {
						$percentage = 15;
						$price = $price;						
						$new_extraprice = ($percentage / 100) * $price;						
							} 
					else if ($setDistance > 80 and $setDistance <= 200 and $price >= 200 and $price < 500) 
						{
						$percentage = 18;
						$price = $price;						
						$new_extraprice = ($percentage / 100) * $price;	
					...
					if($setDistance <= 80 and $price >= 500 and $price < 800) {
						$percentage = 12;
						$price = $price;						
						$new_extraprice = ($percentage / 100) * $price;						
							} 
					else if ($setDistance > 80 and $setDistance <= 200 and $price >= 500 and $price < 800) 
						{
						$percentage = 17;
						$price = $price;						
						$new_extraprice = ($percentage / 100) * $price;	
					...
					if($setDistance <= 80 and $price >= 800 ) {												
						$new_extraprice = $price + 90;						
							} 
					else if ($setDistance > 80 and $setDistance <= 200 and $price >= 800 ) 
						{
						$new_extraprice = $price + 150;
						}	
						
						 $new_extraprice = floatval($new_extraprice); 
						// $new_extraprice = apply_filters('woo_custom_shipping_dbf_calculated_fee', $new_extraprice, $setDistance, $dbf_divider, $price);
				...
				$total_shipping_cost = floatval($set_step_price) + floatval($fixedBasePrice) + floatval($handlingcosts) + floatval($cityFee) + floatval($woo_custom_distance_fee) + floatval($class_amount) /*+ floatval($new_extraprice)*/;
			...
			if (strpos($shipping_id, 'WB_Custom_WooCommerce_Shipping_Method') !== false) {
				...
				if(isset($response['rows'][0]) && $response['rows'][0]['elements'][0]['status'] !== 'NOT_FOUND' && $response['rows'][0]['elements'][0]['status'] !== 'ZERO_RESULTS') {
					...
					if($dbf_maximum_distance != '' && (float)$dbf_maximum_distance <= $setDistance) { 
						//if($otherSettingsArr_details[0][$prefix . '_dbf_limit_hiding'] == 'true') {
							$hide = true;
							add_action( 'woocommerce_after_shipping_rate', 'custom_cart_totals_after_order_total'  );
						//}
					} else {
						//add_action( 'woocommerce_after_shipping_rate', 'custom_cart_totals_after_order_total_empty');
						}
...
function custom_cart_totals_after_order_total_empty(){
		echo '';		
	}
function custom_cart_totals_after_order_total() {	               
			echo '<div class="noticearea">Only Local Pickup is available. Delivery is currently unavailable in your areas. </div>';          
	 }
...
function sendCustomWooShippingDataToAPI($otherSettingsArr_details) {
	...
	$destination = urlencode( $destination_postcode );
